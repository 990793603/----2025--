<template>
	<view class="app">
		...
		<view class="wrapper">
			...
			<view class="reg-wrap row">
				<text class="reg-btn" @click="navTo('/pages/auth/retrievePassword')">忘记密码</text>
				<view class="fill"></view>
				<text>还没有账号？</text>
				<text class="reg-btn" @click="navTo('/pages/auth/register')">立即注册</text>
			</view>
			...
			<view class="other-wrapper">
				...
				<view class="list row">
					...
					<!-- #ifdef APP-PLUS -->
					<view v-if="canUseAppleLogin" class="item column center" style="width: 180rpx;" @click="loginByApple">
...
<script>
	...
	export default{
		...
		onLoad() {
			// #ifdef APP-PLUS
			this.canUseAppleLogin = plus.os.name == "iOS";
			// #endif
		},
		methods: {
			...
			async login(){
				...
				const res = await this.$request('user', 'login', {username,pwd});
				this.$refs.confirmBtn.stop();
